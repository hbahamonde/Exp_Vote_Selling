...
\begin{itemize}
  \item {\color{blue}Sequencing really changes social outcomes, creating losers and winners depending on {\bf who plays first}}.
...
\item \textbf{Intergenerational inequality:} Does inequality persist when \emph{parents invest first} (education, housing), compared to systems where \emph{the state provides support first}?\pause
...
\begin{itemize}
  ...
  \item {\bf Qualitative}: usually study how \textbf{voters begin} the sequence, and {\bf sell their votes}.

  \item {\bf Quantitative}: usually study how \textbf{parties begin} the sequence, and {\bf buy votes}.\pause
...
\begin{itemize}
  \item Integrate {\color{red}vote buying} \emph{and} {\color{blue}vote selling} into the same framework.\pause
  \item {\bf Theory}: formalize a {\bf spatial sequential model} with one voter and two parties.
  ...
  \begin{itemize}
    ...
    \item[\checkmark] Voters earn \textbf{higher payoffs} when parties begin the exchange.
  \end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item When voters begin, they demand higher prices from electorally strong but ideologically distant parties.
  \end{itemize}
...
\begin{columns}[c]
  ...
  \begin{overlayarea}{\textwidth}{4cm}
    \begin{itemize}
      ...
      \item<3-> So: parties target followers cheaply (right), but pay more to buy distant votes (left).
    \end{itemize}
  \end{overlayarea}
...
\begin{columns}[c]
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item When the party is {\bf strong} but distant (voters exploit electorally strong parties' higher electoral stakes {\scriptsize\color{gray}$R_i$}.)
    \end{itemize}
  \end{itemize}
...
\begin{itemize}
  \item \textbf{Who moves first is power.} In many settings---unions vs. governments or parents vs. the welfare state---the side that moves first usually captures more benefits.
  \medskip
  \item \textbf{Same resources, different winners.} Our paper shows that even with the same agents and setup, simply changing who starts the exchange flips who gets the better deal. This logic also travels to other policy contexts. 
  \medskip
