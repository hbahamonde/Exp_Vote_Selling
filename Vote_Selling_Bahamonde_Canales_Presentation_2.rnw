...
\begin{column}{0.55\textwidth}
\begin{itemize}[<+->]
  \item Politics lives on a simple \textbf{left--right ideological space}. {\scriptsize \color{gray}\(\gamma \in \Gamma = \{1,\dots,100\}\)}
  \item There are two parties positioned on that space.\\
  ...
  \item Voters care about {\bf winning the election} and also live on that space.\\
  {\scriptsize \color{gray}Utility \(u_j(\gamma_i)\)}
\end{itemize}
...
\begin{column}{0.55\textwidth}
\begin{itemize}[<+->]
  \item ``Ideological utility'' {\scriptsize \color{gray}\(\Delta\)}
  \item The voter gets more utility {\scriptsize \color{gray}\(\Delta\)} from a closer party {\scriptsize \color{gray}\(u_j(\gamma_i)\)}
\end{itemize}

\medskip
\onslide<3->{
\begin{block}{Implication}
\(\Delta\) represents the minimum compensation the voter needs to switch sides.
\end{block}
}
\end{column}
...
\onslide<3->{
\begin{block}{Implication}
It is cheapest to buy a vote from a \textbf{party supporter} {\scriptsize \color{gray}(\(i^\ast\), with advantage \(\Delta\))}.
\end{block}
}
\end{column}
...
\onslide<3->{
\begin{block}{Implication}
Voters will ask more from parties with \textbf{more to lose} {\scriptsize\color{gray}(big \(R_i\))}.
\end{block}
}
\end{column}
