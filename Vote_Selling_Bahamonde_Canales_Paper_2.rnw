...
# Vote-buying and vote-selling are risky strategies. 

# In this paper we also take distance from classic approaches of clientelism and hypothesize building on the prospect theory approach, as the best theory of decision-making under risk. 

# We hypothesize:
    # 1. Parties OFFER TO buy from core voters because they are NATURALLY (?) risk-averse (IN THE DOMAIN OF?): would hate to lose the sure thing (a voter of their own).
    // because they anticipate the voter's risk-aversion levels (voters would hate to lose the sure thing: ), and gamble their money on the safest bet: buy from their own supporters. 
    # 2. Voters OFFER TO sell to opponent winning parties because they are risk-seeking (IN THE DOMAIN OF?): since voters anticipate the party's risk-aversion levels (parties would hate to lose the sure thing: to win the election), voters gamble their vote in long-shots strategies.
    # Consistent with prospect theory: parties buy votes when,
        # 1. Are likely winners (risk-seeking in the domain of gains).
        # 2. Have experienced sunk costs in the past (risk-aversion in the domain of loses).
